...
AirportFlightStreamReader::CreateStream(uintptr_t buffer_ptr,
  ...
  // So the problem here is that we need the buffers to be 8 byte aligned, but they aren't
  // right now.
  //
  // Ideally we could ensure the alignment in the C++ side before we export.
  //
